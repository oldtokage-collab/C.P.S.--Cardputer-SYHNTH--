# C.P.S. — CardPuter Synth

*[English version here / 英語版はこちら](README.md)*

**M5Stack CardputerADV** 向けの本格DIYシンセサイザーです。PlatformIO + Arduinoフレームワークで開発しています。

Claudeに私のアイデアを伝えコーディングしてもらっています。
Redditや私のTwitterアカウントで開発状況を公開・共有しています。コミュニティからのフィードバックが多くの機能追加のきっかけになりました！
興味を持ってくださったユーザーの皆さん、本当にありがとうございます！

> **現状100%バイブコーディングです！**
> AIの使用やバイブコーディングに抵抗がある方は申し訳ありません！

---

## 🆕 v0.7 アップデート情報

v0.7では音色エディット機能を大幅に強化しました。

- **EDITメニューを VCO / VCF / VCA の3画面に細分化**（旧バージョンの単一EDITメニューを廃止し、より本格的なシンセのような編集体験に）
- **サブオシレーター**（-1oct / -2oct、レベル可変）を追加
- **ノイズブレンド**を追加
- **フィルターのキートラッキング**を追加（音程に応じてカットオフが追従）
- **フィルター専用エンベロープ**（Depth/Attack/Decay/Release）を追加。ビブラート・トレモロとは別に、フィルターだけを動かす独立したエンベロープです
- **汎用LFO**を新設タブとして追加（Sine/Triangle/Sawtooth/Square、Rate 0.1〜20Hz、Depth 0〜100%、Target: Pitch/Volume/Timbre/Filter/PWM）。既存のビブラート・トレモロとは完全に独立して動作します
- **フィルタータイプに「None」（バイパス）を追加**
- **Patch Bank機能を追加**：IMUマッピングやホールド状態を含む全パラメータを、名前を付けて保存・呼び出しできます。リネーム・複製・削除にも対応
- タブ構成: `MAIN → VCO → VCF → VCA → LFO → SETTING`
- キー変更（転調）機能を追加（`[` / `]` キー、±12半音）
- IMU X/Y軸のホールドON/OFF状態が一目でわかる表示を追加

---

## 主な機能

| カテゴリ | 内容 |
|---|---|
| **オシレーター** | リアルタイム・ウェーブテーブル方式：Sine → Triangle → Sawtooth → Square（モーフィング可能）、PWM対応 |
| **サブオシレーター** | -1oct / -2oct、レベル可変 |
| **ノイズ** | ノイズブレンド（レベル可変） |
| **キーボード** | 数字キー `1`〜`0` を C4〜E5 に割り当て。モノフォニック（後着優先） |
| **オクターブ** | `=` / `-` キーで ±2オクターブ |
| **転調（キー変更）** | `[` / `]` キーで ±12半音（オクターブとは独立） |
| **音量** | `,` / `.` キーで5%刻みに調整 |
| **ベンド** | `Z` キー = ベンドダウン、`X` キー = ベンドアップ（ギターのチョーキングのような非対称アタック/リリース） |
| **ADSR** | フルAttack/Decay/Sustain/Releaseエンベロープ、リトリガー対応 |
| **Biquadフィルター** | LPF / HPF / BPF / Notch / **None（バイパス）**。カットオフ(100〜8000Hz)・Q・キートラッキング調整可 |
| **フィルターエンベロープ** | フィルターカットオフ専用のDepth/Attack/Decay/Sustain/Releaseエンベロープ |
| **汎用LFO** | Sine/Triangle/Sawtooth/Square、Rate 0.1〜20Hz、Depth 0〜100%。Pitch/Volume/Timbre/Filter/PWMのいずれかを変調 |
| **ビットクラッシャー** | 16bitから約3bitまでビット深度を落とすローファイエフェクト |
| **ビブラート** | LFOによるピッチ変調（レート・デプス調整可、汎用LFOとは独立） |
| **トレモロ** | LFOによる音量変調（汎用LFOとは独立） |
| **ポルタメント** | ON/OFF、グライド速度調整可。ONにすると、音を弾き直すたびに前の音の高さから今の音の高さへ滑らかに音程が移り変わります（`C`キーでON/OFF切り替え） |
| **ノートホールド** | ONにすると、キーから指を離しても直前に鳴らした音が鳴り続けます（`D`キーでON/OFF切り替え）。IMUで音色を動かしながら演奏したい時などに便利です |
| **IMUマッピング** | BMI270の傾きで各種パラメータを操作可能。X/Y軸それぞれホールドのON/OFF切り替え可 |
| **Patch Bank** | 全パラメータ（IMUマッピング・ホールド状態含む）を名前を付けて保存/呼び出し。リネーム・複製・削除に対応 |
| **SD保存** | 現在の設定を `/CPS/settings.json` に自動保存（SETTING画面からTabキーで戻る際） |

### IMU割り当て可能なターゲット

`NONE` · `TIMBRE` · `VIBRATO_DEPTH` · `VIBRATO_RATE` · `TREMOLO` · `VOLUME` · `PITCH_BEND` · `BEND_UP` · `BEND_DOWN` · `BITCRUSH` · `FILTER_CUTOFF`

- **PITCH_BEND** — バイポーラ：傾きの方向がそのままベンドの方向になります（ランダムピッチ的な効果に最適）
- **BEND_UP / BEND_DOWN** — 絶対値：傾きの量が常にピッチを上げる/下げる方向に作用します

---

## ハードウェア

| 項目 | 値 |
|---|---|
| デバイス | M5Stack CardputerADV |
| MCU | ESP32-S3（デュアルコア Xtensa LX7、240MHz） |
| オーディオ | ES8311コーデック + NS4150Bアンプ、1Wスピーカー、3.5mmジャック |
| IMU | BMI270 6軸（CardputerADVのみ搭載） |
| SDスロット | SPI — SCK=GPIO40, MISO=GPIO39, MOSI=GPIO14, CS=GPIO12 |

> **オリジナル版Cardputer（無印/v1.1）について**：コード上は動作するはずですが、手元に実機がなく動作確認はまだ行っていません。また、IMUセンサーが搭載されていないため、IMUを使った音色コントロールは使用できません。加えて、オリジナル版はハードウェアの制約上キーの同時押しが3キーまでしか対応していないため、4キー以上を同時に押すとフリーズやクラッシュが発生する可能性があります。オリジナル版向けの代替操作の実装はv0.8で対応予定です。

---

## インストール方法

### 方法1 — M5Burnerでインストール（一番簡単・おすすめ）

コンパイル不要で、最も手軽にインストールできる方法です。

1. [M5Burner](https://docs.m5stack.com/en/uiflow/M5Burner) を公式サイトからダウンロード・インストールします
2. M5Stack CardputerADVをUSB-Cでパソコンに接続します
3. M5Burner内で「C.P.S.」（CardPuter Synth）を検索します
4. 対象のCOMポートを選択し、「Burn」（書き込み）ボタンを押します
5. 書き込みが完了すると、CPSが自動的に起動します

> SDカード（FAT32フォーマット）を挿入しておくと、設定の自動保存やPatch Bank機能が使えるようになります。

---

### 方法2 — Launcher FW経由でのインストール

CardputerADVで **Launcher FW** を使用している場合は、コンパイルせずにインストールできます。

1. [Releases](../../releases) ページから最新の `C.P.S.v0.7o.bin` をダウンロードします
2. `.bin` ファイルをSDカードの**ルート直下**にコピーします（サブフォルダに入れないでください）
3. SDカードをCardputerADVに挿入し、Launcher FWで起動します
4. Launcherのファイルブラウザで対象の `.bin` ファイルを選択して書き込みます
5. 書き込み完了後、CPSが自動的に起動します

> FAT32フォーマットのmicro SDカードが、Launcher経由のインストールとCPSの設定保存の両方に必要です。

---

### 方法3 — ソースからビルド（PlatformIO）

#### 必要なもの

- [VSCode](https://code.visualstudio.com/) + **PlatformIO IDE** 拡張機能
- USB-C接続したM5Stack CardputerADV

#### ビルド＆書き込み手順

1. 本リポジトリをクローンまたはダウンロードします
2. VSCodeで `CPS` フォルダを開きます（`File › Open Folder`）
3. `platformio.ini` が自動検出され、初回ビルド時に必要なライブラリが自動でダウンロードされます
4. 画面下部ツールバーの **Upload**（→ボタン）をクリックします

ビルド成功後、`.pio/build/cps/` 内に以下の2ファイルが生成されます。

| ファイル | 用途 |
|---|---|
| `firmware.bin` | アプリ本体のみ（PlatformIOのUploadボタンで使用） |
| `merge.bin` | **統合バイナリ**（bootloader + partitions + app）。M5Burnerや単一ファイル書き込みツールで使用 |

> **書き込みモードに入らない場合**：電源オフ → G0ボタンを押しながら電源オン → G0ボタンを離す

### 初回起動時（インストール方法共通）

初回起動時、SDカード上に自動的に `/CPS/` フォルダが作成されます（FAT32フォーマットのmicro SDカードが必要）。
SETTING画面から抜ける際（Tabキー）に、設定が自動的に `/CPS/settings.json` へ保存されます。
SDカードが挿入されていない場合でも、デフォルト設定でアプリは動作します。

---

## 操作方法

### MAIN画面

| キー | 動作 |
|---|---|
| `1` – `0` | C4〜E5の音を再生 |
| `=` / `-` | オクターブ上げ / 下げ（±2オクターブ） |
| `[` / `]` | 転調（キー変更）下げ / 上げ（±12半音） |
| `,` / `.` | 音量 下げ / 上げ |
| `Z` | ベンドダウン（押している間） |
| `X` | ベンドアップ（押している間） |
| `C` | ポルタメント ON/OFF切り替え |
| `A` | IMU X軸のホールド ON/OFF切り替え |
| `S` | IMU Y軸のホールド ON/OFF切り替え |
| `D` | ノートホールド ON/OFF切り替え |
| `H`（長押し） | ヘルプ表示 |
| デバイスを傾ける | IMU X/Y軸に割り当てたパラメータを操作 |
| `Tab` | VCO画面へ切り替え |

MAIN画面には、現在の音名・周波数・オクターブ/転調/ポルタメント状態、ベンドメーター、IMUの傾き状態、IMU X/Y軸のターゲットと現在値（ホールド中は末尾に **(HOLD)** と表示）が表示されます。

### VCO画面

左列：Timbre・PWM・Detune・FineTune　　右列：Sub Lvl・Sub Oct・Noise

| キー | 動作 |
|---|---|
| `;` / `.` | 項目選択（上下） |
| `,` / `/` | 値の増減 |
| `Tab` | VCF画面へ切り替え |

### VCF画面

左列：Filter（波形タイプ）・Cutoff・Resonance・KeyTrack　　右列：FEnv Depth・Attack・Decay・Release（フィルター専用エンベロープ）

| キー | 動作 |
|---|---|
| `;` / `.` | 項目選択（上下） |
| `,` / `/` | 値の増減 |
| `Tab` | VCA画面へ切り替え |

フィルタータイプは LPF / HPF / BPF / Notch / **None**（バイパス）から選択できます。

### VCA画面（ADSR）

Attack・Decay・Sustain・Release

| キー | 動作 |
|---|---|
| `;` / `.` | 項目選択（上下） |
| `,` / `/` | 値の増減 |
| `Tab` | LFO画面へ切り替え |

### LFO画面

Wave（波形）・Rate（0.1〜20Hz）・Depth（0〜100%）・Target（変調先）

| キー | 動作 |
|---|---|
| `;` / `.` | 項目選択（上下） |
| `,` / `/` | 値の増減 |
| `Tab` | SETTING画面へ切り替え |

画面上部にはLFO波形と、現在の位相を示すリアルタイムのマーカーが表示されます。

### SETTING画面

左列：Patch Save・Patch Load・IMU X軸・IMU Y軸　　右列：Bend幅・Bendアタック速度・Bendリリース速度・Portamento・Porta速度

| キー | 動作 |
|---|---|
| `;` / `.` | 項目選択（上下） |
| `,` / `/` | 値の増減、または Patch Save/Load 画面を開く |
| `Tab` | 設定を保存してMAIN画面へ戻る |

### Patch Bank画面

SETTING画面の「Patch Sv」または「Patch Ld」で `/` キーまたは `,` キーを押すと開きます。

| キー | 動作 |
|---|---|
| `;` / `.` | パッチ選択（上下） |
| `/` または `Enter` | 決定（Load実行 / 新規保存 / 上書き確認） |
| `r` | 選択中のパッチをリネーム |
| `c` | 選択中のパッチを複製 |
| `,` | 選択中のパッチを削除（確認あり） |
| `Tab` | 一段階戻る（名前入力中→一覧、一覧→SETTING画面） |

Saveモードでは一覧の先頭に `<New Patch>` が表示され、新規保存が可能です。パッチは `/CPS/Patch/` フォルダ内にのみ保存され、他のフォルダへは移動できません。

---

## 信号経路

```
オシレーター（ウェーブテーブル・モーフィング）
    │
    ├── サブオシレーター（-1/-2 oct）
    ├── ノイズブレンド
    │
    ▼
ビットクラッシャー
    │
    ▼
Biquadフィルター  ◄── フィルターエンベロープ / フィルターキートラッキング / IMU / 汎用LFO(Filter)
    │
    ▼
音量（キー音量 + IMU音量オフセット + 汎用LFO(Volume)）
    │
    ▼
トレモロ（LFO × デプス）
    │
    ▼
ADSRエンベロープ
    │
    ▼
スピーカー（ES8311 / I2S）
```

ピッチ変調（ビブラートLFO + IMUベンド + キーベンド + 汎用LFO(Pitch)）は、サンプル生成前のオシレーター位相増分に対して適用されます。
汎用LFOのTimbre/PWMターゲットは、オシレーターのモーフ値・PWM幅に対して一時的なオフセットとして適用されます（VCOメニューの保存値自体は変化しません）。

---

## プロジェクト構成

```
CPS/
├── platformio.ini      # ビルド設定
├── merge_bin.py         # ポストビルドスクリプト：M5Burner用merge.binを生成
└── src/
    └── main.cpp          # 全ソースコード（単一ファイル）
```

---

## 依存ライブラリ

PlatformIOが自動管理します。

| ライブラリ | バージョン |
|---|---|
| `m5stack/M5Cardputer` | ≥ 1.1.1 |
| `m5stack/M5Unified` | ≥ 0.2.8 |
| `m5stack/M5GFX` | ≥ 0.2.10 |

`SD` と `SPI` はArduino-ESP32コアに含まれているため、`lib_deps`への追加記述は不要です。

---

## 既知の制限事項 / 今後のアイデア

- モノフォニックのみ（後着優先）。ポリフォニック対応の予定はありません
- IMU軸の感度は固定値。軸ごとの感度調整をSETTING画面に追加予定
- ディスプレイは240×135px。レイアウトはやや窮屈です
- v1.0に向けて新機能を追加予定

---

## ライセンス

MIT — 自由に使用・改変・共有してください。
面白いものを作ったら、ぜひコミュニティにシェアしてください！
