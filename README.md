...
ロードマップのうち、次に着手しやすいのは：

1. **ベンドキー(Z/C)の実装** — 既存のビブラートと同じ「目標値へ滑らかに追従」する
   仕組みをそのまま流用できるため、比較的小さい変更で実装可能
2. **設定値のSD保存/読み込み** — `/CPS`フォルダの土台はできたので、
   次は実際に設定ファイル(JSON等)を読み書きする処理を追加
3. **Tabキーメニュー（プレイ/エディット/設定）** — 3つのうち、まずは中身が一番軽い
   「設定メニュー」（ベンド幅など数値調整のみ）から作るのがおすすめ
